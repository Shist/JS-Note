...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    createBtn.addEventListener("click", ()=>{
        ...
        const isDescriptionWrong = isDescriptionDataWrong(createInputDescription.value);
        if (isTitleWrong) {
    ...
    notesContainer.addEventListener("click", (event)=>{
        ...
        if (event.target.classList.contains("note-card__edit-btn")) prepareEditingModalWindow(event.target.id);
    });
    ...
    formContainer.addEventListener("click", (event)=>{
        if (event.target === formContainer) hideEditingModalWindow();
    });
    document.addEventListener("keydown", (event)=>{
        if (event.code === "Escape" && formContainer.classList.contains("appeared-flex")) hideEditingModalWindow();
    });
    // Model
    ...
    function updateNotesList(arr) {
        const filteredArr = arr.filter((note)=>{
            if (searchSelectNoteState.value === "All" || note.state === searchSelectNoteState.value) return note.title.includes(searchInputTitle.value) && note.description.includes(searchInputDescription.value);
            else return false;
        });
...
//# sourceMappingURL=index.61941409.js.map
